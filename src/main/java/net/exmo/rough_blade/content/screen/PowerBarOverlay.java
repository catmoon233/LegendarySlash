...
public class PowerBarOverlay implements IGuiOverlay {
    ...
    public final static ResourceLocation TEXTURE = new ResourceLocation(Rough_blade.MODID, "textures/gui/icons.png");
    ...
    public static boolean shouldShowManaBar(Player player) {
        ...
        LSVARB.PlayerVariables playerVariables = player.getCapability(LSVARB.PLAYER_VARIABLES_CAPABILITY).orElse(null);
        if (playerVariables == null)
            return false;
        return !player.isSpectator() && display != Display.Never &&
                (display == Display.Always || ((int) playerVariables.playerSimpleVars.get(0).getValue()) < player.getAttributeValue(RBAttribute.Max_Slash_Power.get()) || player.isHolding(itemStack -> ForgeRegistries.ITEMS.getKey(itemStack.getItem()).toString().contains("slashblade")));
