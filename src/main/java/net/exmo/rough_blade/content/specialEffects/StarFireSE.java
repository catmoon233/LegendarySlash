...
import net.exmo.rough_blade.entity.EntityStormSwordsPlus;
...
public class StarFireSE extends SpecialEffectEx {
    ...
    if (event.getSource().getEntity() instanceof Player player){
        if (hasSpecialEffect2(player.getMainHandItem(), RBSpecialEffectRegistry.STAR_FIRE.getId(),player)){
    ...
    if (event.getSource().getDirectEntity() instanceof EntityStormSwords entityStormSwords){
    ...
    public static void OnHit(SlashBladeEvent.HitEvent event){
        ...
        if (user instanceof Player player){
            if (!SpecialEffectEx.hasSpecialEffect2(event.getBlade(), RBSpecialEffectRegistry.STAR_FIRE.getId(), player))return;
            ...
            if (persistentData.contains("Star_tag")){
                if (persistentData.getInt("Star_tag") >= 10){
                    ...
                    for (int i = 0; i < count; i++) {
                            EntityStormSwordsPlus ss = new EntityStormSwordsPlus(RBEntityRegistry.STORMSOWRDPLUS,
                                    worldIn);
                            ...
                            ss.setRoll(0);
                            ss.setDamage(((getPlayerDamage(user)+2)+persistentData.getDouble("totalDamage")*0.2)*2f);
                            // force riding
...
//                                entityDrive.setDeltaMovement(motion);
    ...
    public static void DoSlash(SlashBladeEvent.DoSlashEvent event) {
        ...
        if (user instanceof Player player) {
            ...
            if (!SpecialEffectEx.hasSpecialEffect2(mainHandItem, RBSpecialEffectRegistry.STAR_FIRE.getId(), player))return;
