...
public class SlashBladeRender implements IItemDecorator {
    ...
    public boolean render(GuiGraphics guiGraphics, Font font, ItemStack stack, int xOffset, int yOffset) {
        return renders(guiGraphics, font, stack, xOffset, yOffset);
    }
    ...
    public  boolean renders(GuiGraphics guiGraphics, Font font, ItemStack stack, int xOffset, int yOffset) {
        ...
        if (!Screen.hasShiftDown() || NEW_TEXT_RENDER_ALAWAYS.get() && !OLD_TEXT_RENDER_ALAWAYS.get())return false;
            ...
            if (totalVisible < 2) return false;
            ...
            guiGraphics.drawString(
                    ...
                    0xFFFFFF, // 修改：白色(0xFFFFFF) → 红色(0xFF0000)
                    true // 必须启用样式解析
            );
