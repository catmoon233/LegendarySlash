package net.exmo.legendary_slash.content.screen;
...
public class PowerBarOverlay implements IGuiOverlay {
    public static final PowerBarOverlay instance = new PowerBarOverlay();

    public final static ResourceLocation TEXTURE = new ResourceLocation(Legendary_slash.MODID, "textures/gui/icons.png");
    ...
    static final int TEXT_COLOR = ChatFormatting.RED.getColor();
    ...
    public void render(ForgeGui gui, GuiGraphics guiHelper, float partialTick, int screenWidth, int screenHeight) {
        ...
        int maxMana = (int) player.getAttributeValue(LSAttribute.Max_Slash_Power.get());
    ...
    public static boolean shouldShowManaBar(Player player) {
        ...
        return !player.isSpectator() && display != Display.Never &&
                (display == Display.Always || ((int) player.getCapability(LSVARB.PLAYER_VARIABLES_CAPABILITY).orElse(null).playerSimpleVars.get(0).getValue()) < player.getAttributeValue(LSAttribute.Max_Slash_Power.get()) || player.isHolding(itemStack -> ForgeRegistries.ITEMS.getKey(itemStack.getItem()).toString().contains("slashblade")));
